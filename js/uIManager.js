...
class UIManager {
    ...
    setMapLegendAndDownloadLinks(filename, map) {
        ...
        const urlLegend = `${this.webHost}volcanoes/${filename}/imgs/legend_${map}.png`;
        ...
        if (downloadButtonDiv || downloadButton || downloadButton2) {
            ...
            downloadButton.href = `${this.webHost}volcanoes/${filename}/downloads/all.zip`;
            downloadButton2.href = `${this.webHost}volcanoes/${filename}/downloads/${map}.zip`;
        } else {
    ...
    setMapList(response, filename) {
        ...
        response.data.forEach(mapItem => {
            ...
            link.id = mapItem.map; //.name;
            link.className = "map-but";
            link.textContent = formattedMapName.map;
            link.href = "#";
            ...
            link.addEventListener('click', (event) => {
                ...
                this.loadMap(filename, mapItem.map); // Load the selected map.
            });
    ...
    setActiveVolcano(volcanoName, latitude = 'No specified', longitude = 'No specified') {
        ...
        this.volcanoAPIManager.getVolcanoAvailableMaps(volcanoName, response => {
            ...
            const kmlPath = `http://multiriskprevention.com/volcanoes/${volcanoName}/kml/preview.kml`;
	...
	this.activateMap(map);
        ...
        const kmlPath = `http://multiriskprevention.com/volcanoes/${filename}/kml/${map}/doc.kml`;
        this.mapManager.addKmlLayer(kmlPath);
    }
